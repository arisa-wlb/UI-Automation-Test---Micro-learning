...
test('กรอกข้อมูล Payment แนบ slip ด้วยนามสุกลไฟล์ .png และ Confirm Payment สำเร็จ', async ({ page }) => {
  ...
  await test.step('อัปโหลดรูปภาพ sample-slip.png', async () => {
    const imagePath = path.join(__dirname, '..', 'assets' , 'png' , 'sample-slip.png');
    await page.getByTestId('slip-file-input').setInputFiles(imagePath);
  });
        
  await test.step('ตรวจสอบรูปภาพ sample-slip.png preview ', async () => {
    await expect(page.getByTestId('slip-image-preview')).toBeVisible();
  });
  ...
  await test.step('ตรวจสอบ Filename เท่ากันกับ sample-slip.png', async () => {
    await expect(page.getByTestId('success-filename')).toHaveText('sample-slip.png');
  }); 
